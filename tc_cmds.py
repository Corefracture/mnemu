...
def tc_clear_root_qdisc(iface):
    stdout, stderr = (tc_execute_cmd(TC_QDISC, TC_DEL, TC_DEV, iface, TC_ROOT))

    log_raw_output(stdout, stderr)


def tc_create_root_tokenbucket_qdisc(iface, qdiscid):
    stdout, stderr = tc_execute_cmd(TC_QDISC, TC_ADD, TC_DEV, iface, TC_ROOT, TC_HNDL, qdiscid + ":", "htb")
    log_raw_output(stdout, stderr)
...
def tc_create_ip_traffic_class(iface, classid, parent_qdiscid, rate_kbit):
    ...
    stdout, stderr = tc_execute_cmd(TC_CLASS, TC_ADD, TC_DEV, iface, TC_PARNT, parent_qdiscid, TC_CLASSID,
...
def tc_update_ip_traffic_class(iface, classid, parent_qdiscid, rate_kbit):
    parent_qdiscid = parent_qdiscid + ":"
    classid = parent_qdiscid + classid
    stdout, stderr = tc_execute_cmd(TC_CLASS, TC_CHANGE, TC_DEV, iface, TC_PARNT, parent_qdiscid, TC_CLASSID,
                                    classid, TC_HTB, TC_RATE, rate_kbit + "kbit")
...
def tc_create_ip_filter(iface, ip, prnt_qdisc_id, prnt_class_id, ip_is_dst=False):
    ...
    stdout, stderr = tc_execute_cmd(TC_FILTER, TC_ADD, TC_DEV, iface, TC_PARNT, prnt_qdisc_id,
